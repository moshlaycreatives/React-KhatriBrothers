...
// //       const dateTime = new Date(`${date}T${hours}:${minutes}:00`);
// //       // Convert to ISO string but keep local time by not adding timezone offset
...
//       // Construct the date-time string in local time
//       // Create a Date object to ensure time formatting is correct
//       const dateTimeLocal = new Date(`${date}T${hours}:${minutes}:00`);

//       // Convert to local string format
//       // We will send this format to avoid unexpected timezone issues
//       const localDateTimeString = dateTimeLocal.toISOString().slice(0, -1);

//       // For debugging: Log the constructed local time and the ISO string
//       console.log("Local Date-Time String:", localDateTimeString);

//       return localDateTimeString;
//     };

//     // Convert date, startTime, and endTime to local ISO format
//     const startDateTime = formatDateTime(formValues.date, formValues.startTime);
//     const endDateTime = formatDateTime(formValues.date, formValues.endTime);
...
const AddClass = () => {
  ...
  const handleSubmit = async () => {
    const formatDateTime = (date, time) => {
      ...
      const dateTimeLocal = new Date(`${date}T${hours}:${minutes}:00`);
      // Adjust to local time without timezone offset
    ...
    const formData = {
      ...
      date: startDateTime, // Use the formatted local time
      startTime: startDateTime,
      endTime: endDateTime,
    };
