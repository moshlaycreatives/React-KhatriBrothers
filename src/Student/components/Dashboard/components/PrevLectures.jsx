...
//                 const { formattedTime: formattedEndTime } = formatDateAndTime(
//                   row.endTime
//                 );
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        const data = res.data.data.map((row) => {
          ...
          return { ...row, endTime }; // Attach the new endTime to each lecture
        });
        setClassData(data);
      } catch (err) {
        console.error("Failed to fetch previous classes:", err);
      } finally {
        setLoading(false);
      }
    };
      ...
      <Box>
        ...
        >
          <Table size="small" aria-label="a dense table">
            ...
            <TableBody>
              {classData.map((row, index) => {
                const { formattedDate } = formatDateAndTime(row.startTime);
                const { formattedTime: formattedStartTime } = formatDateAndTime(row.startTime);
                const { formattedTime: formattedEndTime } = formatDateAndTime(row.endTime);

                const now = new Date();
                const isPastEndTime = now > row.endTime; // Check if the lecture has ended
