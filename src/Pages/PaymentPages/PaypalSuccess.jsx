...
import { EnrollCustomer, EnrollPaypalCustomer } from '../../store/actions/authActions';
...
const PaypalSuccess = () => {
  ...
  const enrollCustomer = localStorage.getItem('courseId');
  ...
  const customerId = localStorage.getItem('paypalcustomerId');
  ...
  const addenroll = async () => {
    try {
      ...
      localStorage.removeItem('paypalcustomerId');
  ...
  const enroll = async () => {
    try {
      ...
      await dispatch(EnrollPaypalCustomer(customerId))
      addenroll()
